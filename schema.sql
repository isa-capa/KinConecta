...
-- =========================================================
-- MODELO RELACIONAL (RELACIONES Y CARDINALIDADES)
-- =========================================================
-- Notación:
--   (1) ----< (N)  = relación uno a muchos
--   (1) ---- (1)   = relación uno a uno
--   (N) >----< (N) = relación muchos a muchos (mediante tabla puente)
--
-- 1) Núcleo de usuarios y perfiles
--   languages (1) ----< (N) users                      [fk_users_language]
--   users (1) ----< (N) auth_sessions                  [fk_auth_sessions_user]
--   users (1) ---- (1) tourist_profiles                [fk_tourist_profiles_user]
--   users (1) ---- (1) guide_profiles                  [fk_guide_profiles_user]
--
-- 2) Idiomas, intereses y expertise (N:M con tablas puente)
--   tourist_profiles (N) >----< (N) languages
--     vía tourist_profile_languages                    [fk_tpl_user, fk_tpl_language]
--   guide_profiles (N) >----< (N) languages
--     vía guide_profile_languages                      [fk_gpl_user, fk_gpl_language]
--   tourist_profiles (N) >----< (N) interests
--     vía tourist_profile_interests                    [fk_tpi_user, fk_tpi_interest]
--   guide_profiles (N) >----< (N) guide_expertise_areas
--     vía guide_profile_expertise                      [fk_gpe_user, fk_gpe_expertise]
--
-- 3) Extensiones del perfil de guía
--   guide_profiles (1) ----< (N) guide_locations       [fk_guide_locations_user]
--   guide_profiles (1) ----< (N) guide_certifications  [fk_guide_certifications_user]
--   guide_profiles (1) ----< (N) guide_adaptations     [fk_guide_adaptations_user]
--
-- 4) Catálogo de tours y destinos
--   users (guide) (1) ----< (N) tours                  [fk_tours_guide]
--   tour_categories (1) ----< (N) tours                [fk_tours_category]
--   tours (1) ----< (N) tour_included_items            [fk_tour_included_items_tour]
--   tours (N) >----< (N) destinations
--     vía tour_destinations                            [fk_tour_destinations_tour, fk_tour_destinations_destination]
--
-- 5) Reservas, historial y calendario
--   tours (1) ----< (N) trip_bookings                  [fk_trip_bookings_tour]
--   users (tourist) (1) ----< (N) trip_bookings        [fk_trip_bookings_tourist]
--   users (guide) (1) ----< (N) trip_bookings          [fk_trip_bookings_guide]
--   trip_bookings (1) ----< (N) trip_status_history    [fk_trip_status_history_trip]
--   users (1) ----< (N) trip_status_history            [fk_trip_status_history_user]
--   users (guide) (1) ----< (N) guide_calendar_events  [fk_guide_calendar_guide]
--   trip_bookings (1) ----< (N) guide_calendar_events  [fk_guide_calendar_trip]
--
-- 6) Favoritos
--   users (tourist) (N) >----< (N) users (guide)
--     vía favorite_guides                              [fk_favorite_guides_tourist, fk_favorite_guides_guide]
--   users (tourist) (N) >----< (N) tours
--     vía favorite_tours                               [fk_favorite_tours_tourist, fk_favorite_tours_tour]
--
-- 7) Reseñas
--   trip_bookings (1) ----< (N) reviews                [fk_reviews_trip]
--   tours (1) ----< (N) reviews                        [fk_reviews_tour]
--   users (guide) (1) ----< (N) reviews                [fk_reviews_guide]
--   users (tourist) (1) ----< (N) reviews              [fk_reviews_tourist]
--   reviews (1) ---- (0..1) review_replies             [fk_review_replies_review + uq_review_replies_review]
--   users (guide) (1) ----< (N) review_replies         [fk_review_replies_guide]
--
-- 8) Chat y notificaciones
--   trip_bookings (1) ----< (N) chat_threads           [fk_chat_threads_trip]
--   users (tourist) (1) ----< (N) chat_threads         [fk_chat_threads_tourist]
--   users (guide) (1) ----< (N) chat_threads           [fk_chat_threads_guide]
--   chat_threads (1) ----< (N) chat_messages           [fk_chat_messages_thread]
--   users (1) ----< (N) chat_messages                  [fk_chat_messages_sender]
--   users (1) ----< (N) notifications                  [fk_notifications_user]
--
-- 9) Soporte y FAQ
--   users (1) ----< (N) support_tickets                [fk_support_tickets_user]
...
--
-- 11) Perfilador / matching
--   users (1) ----< (N) compatibility_profiles         [fk_compatibility_profiles_user]
--   compatibility_profiles (1) ----< (N) compatibility_answers [fk_compatibility_answers_profile]
--
...
--   y además documenta explícitamente relaciones y cardinalidades del modelo.
-- =========================================================
