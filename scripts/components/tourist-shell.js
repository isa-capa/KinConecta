(function () {
  ...
  async function runLogout() {
    try {
      const maybeLogout = window.KCAuthApi?.auth?.logout || window.KCTouristApi?.auth?.logout;
  ...
  function setupTopbarUserMenu() {
    ...
    const avatarUrl = TOURIST_AVATAR_ASSET;
  ...
  function setupChatWidget(container) {
    ...
    const threadData = {
      empty: [
        {
          from: "guest",
          text: "Aquí verás tus conversaciones cuando el backend tenga mensajes disponibles.",
        },
      ],
    };
    ...
    const resolveMessageAuthor = (from) => {
      ...
      return threadProfiles[activeThread] || { name: "Guía local", avatarUrl: GUIDE_AVATAR_ASSET };
    };
