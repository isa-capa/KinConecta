(function () {
  ...
  function resolveProfilerWizardPath() {
    ...
    return "./pages/profiler/profiles-wizard.html";
  }
  ...
  function persistTemporarySession(user) {
    ...
    if (session.fullName) {
      localStorage.setItem(AUTH_FULL_NAME_KEY, session.fullName);
    }
  ...
  function persistBackendSession(result) {
    ...
    if (user.fullName) {
      localStorage.setItem(AUTH_FULL_NAME_KEY, String(user.fullName));
    }
