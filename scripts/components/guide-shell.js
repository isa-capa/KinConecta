(function () {
  ...
  function getSessionSnapshot() {
    return window.KCAuthState?.getSnapshot?.() || null;
  }
  ...
  async function runLogout() {
    try {
      const maybeLogout = window.KCAuthApi?.auth?.logout || window.KCGuideApi?.auth?.logout;
  ...
  function setupTopbarUserMenu() {
    ...
    const fullName = resolveDisplayName(session?.fullName, sidebarUserName);
    const roleLabel = "GUIA";
    const initials = getInitials(fullName);
    const avatarUrl = GUIDE_AVATAR_ASSET;
  ...
  function applySidebarUser(snapshot) {
    const userNameNode = document.getElementById("userName");
    const avatarNode = document.querySelector(".sidebar__avatar");
    const fullName = resolveDisplayName(snapshot?.fullName, snapshot?.name);
    const avatarUrl = GUIDE_AVATAR_ASSET;

    if (userNameNode) {
      userNameNode.textContent = fullName;
    }

    if (avatarNode) {
      avatarNode.setAttribute("aria-label", `Avatar de ${fullName}`);
      avatarNode.textContent = "";
      avatarNode.style.backgroundImage = avatarUrl ? `url('${avatarUrl}')` : "";
  ...
  async function syncCurrentUserUi() {
    const snapshot = (await window.KCAuthState?.getCurrentUser?.()) || getSessionSnapshot();
    applySidebarUser(snapshot);
  }
  ...
  function resolveTemporaryAvatar(_seed) {
    return TOURIST_AVATAR_ASSET;
  }
  ...
  function setupChatWidget(container) {
    ...
    const threadData = {
      empty: [
        {
          from: "guest",
          text: "Aquí verás tus conversaciones cuando existan mensajes reales.",
        },
      ],
    };
    ...
    const currentUserProfile = (() => {
      let displayName = "Tu perfil";
      try {
        const raw = localStorage.getItem("kc_temp_auth_session_v1");
        const session = raw ? JSON.parse(raw) : null;
        displayName = resolveDisplayName(session?.fullName, session?.name, displayName);
      } catch (_error) {
    ...
    const resolveMessageAuthor = (from, item) => {
      ...
      return threadProfiles[activeThread] || { name: "Turista", avatarUrl: TOURIST_AVATAR_ASSET };
    };
    ...
    const hydrateMessagesForThread = async (threadKey) => {
      ...
      try {
        ...
        console.warn("No se pudieron cargar los mensajes del chat.", error);
      }
    };
  ...
  function setupNotifications() {
    ...
    const renderNotifications = () => {
      ...
      if (!notifications.length) {
        listEl.innerHTML = `
          <li class="guide-notifications__item guide-notifications__item--empty">
            <p class="guide-notifications__item-title">No tienes notificaciones nuevas.</p>
            <p class="guide-notifications__item-meta">Todo está al día.</p>
          </li>
        `;
        return;
      }
