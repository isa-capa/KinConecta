const TouristTripsApp = (() => {
  ...
  async function hydrateFromApi() {
    ...
    try {
      ...
      console.warn("No se pudieron cargar los viajes desde la API.", error);
      state.trips = fallbackTrips.slice();
      state.featuredTrip = state.trips[0];
    }
  }
  ...
  async function hydrateTripDetail(trip) {
    ...
    try {
      ...
      trip.guideAvatar = GUIDE_AVATAR_ASSET;
      trip.image = detail.imageUrl || detail.image || trip.image;
    } catch (error) {
      console.warn("No se pudieron cargar los detalles del viaje.", error);
    }
  }
  ...
  function renderFeatured() {
    ...
    if (!trip) {
      ...
      if (dom.featuredTitle) dom.featuredTitle.textContent = "Aún no tienes un viaje próximo";
      if (dom.featuredLocation) dom.featuredLocation.textContent = "Cuando reserves una experiencia, la verás aquí.";
      if (dom.featuredDate) dom.featuredDate.textContent = "Sin fecha programada";
      if (dom.featuredGuideName) dom.featuredGuideName.textContent = "Sin guía asignado";
      if (dom.featuredGuideAvatar) dom.featuredGuideAvatar.src = GUIDE_AVATAR_ASSET;
      if (dom.btnFeaturedChat) dom.btnFeaturedChat.disabled = true;
      return;
    }
