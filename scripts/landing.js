(function () {
  ...
  const canUseProtectedExploreApi = () => {
    if (!window.KCTouristApi?.explore) return false;
    const token = String(window.localStorage.getItem("kcAuthToken") || "").trim();
    if (token) return true;
    return Boolean(window.KCAuthState?.getSnapshot?.().isAuthenticated);
  };
  ...
  const normalizeText = (value) =>
    String(value || "")
      ...
      .replace(/\s+/g, " ")
      .trim();
  const landingTourImage = (...candidates) => {
    const direct = candidates.find((value) => {
      const raw = String(value || "").trim();
      if (!raw) return false;
      if (/kinconecta\.test/i.test(raw)) return false;
      if (/\/t[1-5]\.jpg$/i.test(raw)) return false;
    ...
    const tourAssetUrl = (fileName) =>
      new URL(`./assets/tours/categories/${fileName}`, window.location.href).href;
    if (/gastronom|food|culin|mercad|cantina/.test(haystack)) {
      return tourAssetUrl("gastronomia.jpeg");
    }
    if (/aventur|sender|montan|nature|naturaleza|cenote|ecotur/.test(haystack)) {
      return tourAssetUrl("naturaleza.jpeg");
    }
    if (/hist|cultura|arque|architecture|arquitect|museo|centro/.test(haystack)) {
      return tourAssetUrl("historia_y_cultura.jpeg");
    }
    if (/arte|art|street|mural|fotografia|photo/.test(haystack)) {
      return tourAssetUrl("arte_urbano.jpeg");
    }
    if (/playa|beach|mar|coast|isla/.test(haystack)) {
      return tourAssetUrl("naturaleza.jpeg");
    }
    return tourAssetUrl("arquitectura.jpeg");
  };
  ...
  const guideAssetUrl = () =>
    new URL("./assets/users/user_guide.png", window.location.href).href;
      ...
      .map((tour) => {
        ...
        return `
          <article class="landing-result-card landing-result-card--tour">
            <div class="landing-result-card__media">
              <img class="landing-result-card__media-img" src="${toSafeUrl(tour.image)}" alt="${escapeHtml(tour.title)}" loading="lazy" />
            </div>
          ...
          <article class="landing-result-card landing-result-card--guide">
            <div class="landing-result-card__body">
              <div class="landing-result-card__guide-head">
                <div class="landing-result-card__avatar">
                  <img class="landing-result-card__avatar-img" src="${toSafeUrl(guide.avatar)}" alt="${escapeHtml(guide.name)}" loading="lazy" />
                </div>
